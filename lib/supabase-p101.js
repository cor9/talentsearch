...
export async function findInviteById(inviteId) {
  const rows = await dbSelect(
    'p101_opencall_rep_invites',
    { 'id': `eq.${inviteId}` },
    { select: 'id,event_id,rep_name,rep_email,rep_agency,rep_role,registered_via,expires_at,revoked_at' }
  )
  return rows[0] ?? null
}
...
export async function findApplicationForRep(applicationId, eventId) {
  const rows = await dbSelect(
    ...
    { select: 'id,event_id,status,actor_name,guardian_name,guardian_email,guardian_phone,is_seed' }
  )
  return rows[0] ?? null
}
...
export async function createIntroRequest(inviteId, eventId, applicationId, requester) {
  ...
  const rows = await dbInsert('p101_opencall_intro_requests', {
    ...
    requester_agency: requester.agency || null,
...
// ─── Introductions (list + contact reveal) ───────────────────────────────────

export async function getIntroRequests(inviteId) {
  return dbSelect(
    'p101_opencall_intro_requests',
    { invite_id: `eq.${inviteId}` },
    { select: 'id,application_id,status,requested_at,contact_revealed_at', order: 'requested_at.desc' }
  )
}

// Guardian contact for applications the invite has ALREADY requested an
// introduction to. Only call after confirming the intro request exists.
export async function getGuardianContacts(eventId, applicationIds) {
  if (!applicationIds.length) return []
  return dbSelect(
    'p101_opencall_applications',
    { event_id: `eq.${eventId}`, id: `in.(${applicationIds.join(',')})`, status: 'eq.submitted' },
    { select: 'id,actor_name,guardian_name,guardian_email,guardian_phone' }
  )
}

export async function markContactRevealed(introRequestId) {
  await dbUpdate(
    'p101_opencall_intro_requests',
    { id: `eq.${introRequestId}`, contact_revealed_at: 'is.null' },
